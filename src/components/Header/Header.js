...
function Header({openBurg, loggedIn}) {
  return (
    <header className="header">
      <Link className="header__logo" to="/"><img src={logo} alt="Логотип"/></Link>
      <div className={`header__is-log ${loggedIn ? '' : 'no-display'}`}>
      ...
      <img onClick={openBurg} src={menuIcon} alt="Меню"
           className={`header__hamburger ${loggedIn ? '' : 'no-display'}`}/>
      <div className={`header__no-log ${loggedIn ? 'no-display' : ''}`}>
